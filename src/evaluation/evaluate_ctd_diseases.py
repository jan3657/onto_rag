...
MESH_SHORT_ID_RE = re.compile(r"^[CD]\d+$")


def normalize_ctd_id(raw_id: Optional[str]) -> Optional[str]:
    """Normalize CTD/NCBI Disease IDs into stable CURIE form when possible."""
    if not raw_id:
        return None
    ...
    if rid.upper().startswith("MESH:"):
        return "MESH:" + rid.split(":", 1)[1]
    if rid.upper().startswith("OMIM:"):
        return "OMIM:" + rid.split(":", 1)[1]

    # Many NCBI disease records use bare C/D IDs that are MeSH disease IDs.
    if MESH_SHORT_ID_RE.match(rid.upper()):
        return f"MESH:{rid.upper()}"

    return rid


def build_ctd_id_alias_map(tsv_path: Path) -> Dict[str, Set[str]]:
    """
    Build alias map from CTD disease file:
    alias/alt_id -> canonical DiseaseID (usually MESH:...).
    """
    alias_map: Dict[str, Set[str]] = {}
    if not tsv_path.exists():
        logger.warning("CTD ontology TSV not found for alias mapping: %s", tsv_path)
        return alias_map
    ...
    with tsv_path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            canonical_id = normalize_ctd_id(cols[1].strip())
            if not canonical_id:
                continue

            # Canonical self-map keeps downstream expansion logic simple.
            alias_map.setdefault(canonical_id, set()).add(canonical_id)

            alt_ids_raw = cols[2].strip()
            if not alt_ids_raw:
                continue

            for alt in alt_ids_raw.split("|"):
                alt_id = normalize_ctd_id(alt.strip())
                if not alt_id:
                    continue
                alias_map.setdefault(alt_id, set()).add(canonical_id)

    canonical_count = sum(1 for k, v in alias_map.items() if k in v)
    logger.info(
        "Built CTD ID alias map from %s: %d keys (%d canonical self-keys)",
        tsv_path,
        len(alias_map),
        canonical_count,
    )
...
def expand_ids_for_eval(ids: Set[str], id_alias_map: Dict[str, Set[str]]) -> Set[str]:
    """
    Expand IDs with normalized and canonical-equivalent forms for evaluation.
    """
    expanded: Set[str] = set()
    for raw_id in ids:
        nid = normalize_ctd_id(raw_id)
        if not nid:
            continue
        expanded.add(nid)
        expanded.update(id_alias_map.get(nid, set()))
    return expanded
...
def run_ingestion(max_rows: Optional[int] = None) -> None:
    """Build indexes for CTD Diseases with both MiniLM and SapBERT embeddings."""
    ...
    print(f"\n{'='*60}")
    print(f"INGESTING: CTD Diseases with DUAL-VECTOR")
    print(f"{'='*60}")

    print(f"\n[1/6] Parsing CTD_diseases.tsv...")
...
def load_dataset(input_path: Path, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    with gzip.open(input_path, 'rt', encoding='utf-8') as f:
        for line in f:
            ...
            if context_left or context_right:
                ...
                # Fallback for datasets that store combined context in one field.
    ...
    total = len(items)
    context_rate = (context_nonempty / total) if total else 0.0
    logger.info(
        f"Loaded {total} items from {input_path} "
        f"(non-empty context: {context_nonempty}/{total}, {context_rate:.2%})"
    )
    if total > 0 and context_nonempty == 0:
        logger.warning(
            "All loaded items have empty context. To use context-aware prompting, "
            "rebuild/supply the dataset with context_left/context_right (or context) fields."
        )
    return items
...
async def evaluate_item(
    ...
    try:
        ...
        eval_gold_ids = expand_ids_for_eval(gold_ids, id_alias_map)
        ...
        pred_id_raw = mapping_result.get("id") if isinstance(mapping_result, dict) else None
        pred_id = normalize_ctd_id(pred_id_raw)
        pred_eval_ids = expand_ids_for_eval({pred_id}, id_alias_map) if pred_id else set()

        # Extract predicted sources
        predicted_sources = []
        if pred_id and candidates:
            for cand in candidates:
                if normalize_ctd_id(cand.get("id")) == pred_id:
                    predicted_sources = cand.get("retrieval_sources", [])
                    break

        # Correctness check with normalization + alias resolution (OMIM/MeSH crosswalk).
        is_correct = bool(pred_eval_ids & eval_gold_ids) if pred_id else False
        elapsed_time = time.perf_counter() - start_time
        candidate_ids = [normalize_ctd_id(c.get("id")) for c in candidates[:10]] if candidates else []
        candidate_ids = [cid for cid in candidate_ids if cid]
        gold_in_candidates = False
        if candidate_ids:
            candidate_eval_ids: Set[str] = set()
            for cid in candidate_ids:
                candidate_eval_ids.update(expand_ids_for_eval({cid}, id_alias_map))
            gold_in_candidates = bool(eval_gold_ids & candidate_eval_ids)

        return {
            "query": query,
            "context": context[:200] if context else "",
            "gold_ids": list(gold_ids),
            "gold_ids_for_eval": sorted(eval_gold_ids),
            "predicted_id": pred_id,
            "predicted_id_raw": pred_id_raw,
        ...
        return {
            ...
            "context": item.get("context", "")[:200] if item.get("context") else "",
            "is_correct": False,
            "error": str(e),
...
async def main():
    ...
    id_alias_map = build_ctd_id_alias_map(config.ONTOLOGIES_CONFIG[ONTOLOGY_KEY]["path"])
    ...
    try:
        ...
        tasks = [evaluate_item(pipeline, i, semaphore, cache, id_alias_map) for i in items]
