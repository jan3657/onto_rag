...
Usage:
  ...
  python -m src.evaluation.evaluate_cafeteria_foodon --ingest --ingest-limit 200  # Quick ingest test
...
def run_ingestion(max_rows: Optional[int] = None) -> None:
    """Build indexes for FoodOn with both MiniLM and SapBERT embeddings."""
    ...
    print(f"\n{'='*60}")
    print(f"INGESTING: FoodOn (OWL) with DUAL-VECTOR")
    print(f"{'='*60}")
    if max_rows:
    ...
    print(f"\n[1/6] Parsing foodon.owl (this may take time)...")
