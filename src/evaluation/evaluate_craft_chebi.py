...
Usage:
  ...
  python -m src.evaluation.evaluate_craft_chebi --ingest --ingest-limit 200  # Quick ingest test
...
def run_ingestion(max_rows: Optional[int] = None) -> None:
    """Build indexes for ChEBI with both MiniLM and SapBERT embeddings."""
    ...
    print(f"\n{'='*60}")
    print(f"INGESTING: ChEBI (OWL) with DUAL-VECTOR (MiniLM + SapBERT)")
    print(f"{'='*60}")
    if max_rows:
        print(f"Testing mode: limiting ontology dump to first {max_rows} labeled terms")

    # Ensure directory exists
    cfg['dump_json_path'].parent.mkdir(parents=True, exist_ok=True)

    print(f"\n[1/6] Parsing chebi.owl (this may take time)...")
    ...
    print(f"\n[5/6] Building SapBERT embeddings (this will take longer)...")
    build_embeddings(
        cfg['dump_json_path'],
...
async def main():
    ...
    parser.add_argument("--ingest-limit", type=int, default=None,
                        help="Limit ontology terms during ingestion (for testing)")
    ...
    print(f"\n{'='*60}\nEVALUATING: CRAFT ChEBI ({args.provider})\n{'='*60}")
