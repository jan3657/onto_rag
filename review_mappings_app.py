...
def prepare_dataframe(mapped_ingredients: List[Dict]) -> pd.DataFrame:
    ...
    for i, item in enumerate(mapped_ingredients):
        ...
        record = {
            ...
            "Ontology Term": mapping.get('label', '⚠️ UNMAPPED') if is_mapped else '⚠️ UNMAPPED',
            "Ontology ID": mapping.get('id', 'N/A') if is_mapped else 'N/A',
            "Confidence": mapping.get('confidence_score', 0.0) if is_mapped else 0.0,
...
if data:
    ...
    # *** CHANGE: Used st.code for better visibility of the ingredients string ***
    ...
    if not mapped_ingredients_list:
        ...
        # --- Selection Handling and Details Display for AgGrid ---
        selected_row_data = None
        
        # *** CHANGE: New logic to default to the first row if nothing is selected ***
        ...
        # If no row is clicked and the list isn't empty, default to the first ingredient
        elif mapped_ingredients_list:
            selected_row_data = mapped_ingredients_list[0]

        # If we have data (either from a click or the default), display the details
